...
import US_GCPCC as core


# =========================
# 0) Paths
# =========================
EDGE_DIR = core.EDGE_DIR
NODE_PATH = core.NODE_PATH

SCENARIO_FILES = dict(core.SCENARIO_FILES)
SCENARIO_DAYS = dict(core.SCENARIO_DAYS)
...
# =========================
SCALE_LABEL = dict(core.SCALE_LABEL)

SCALE_ID_TO_SEG: Dict[int, Tuple[int, int, int]] = dict(core.SCALE_ID_TO_SEG)
SEG_TO_SCALE_ID: Dict[Tuple[int, int, int], int] = dict(core.SEG_TO_SCALE_ID)


def id_to_seg(scale_id: int) -> Tuple[int, int, int]:
    return tuple(SCALE_ID_TO_SEG[int(scale_id)])
...
C_sess_day = dict(core.C_sess_day)
build_cost = dict(core.build_cost)
op_cost_yr = dict(core.op_cost_yr)


# =========================
# 2) Knobs (PSO baseline)
# =========================
EDGE_WEIGHT_MIN = float(core.EDGE_WEIGHT_MIN)
BETA_PENALTY = float(core.BETA_PENALTY)
...
# terminal progress knobs
PRINT_EVERY_IT = 1
PRINT_EVERY_FE_HINT = SAVE_EVERY_FE
...
def read_nodes(node_path: str) -> pd.DataFrame:
    return core.read_nodes(node_path)


def read_edges(edge_path: str) -> pd.DataFrame:
    return core.read_edges(edge_path)
...
def filter_edges_and_get_active_nodes(
    ...
    return core.filter_edges_and_get_active_nodes(
        all_nodes=all_nodes,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run PSO global baseline.")
