...
LOG_DIR = os.path.join(SCRIPT_DIR, "de_global_logs")
...
# DE parameters (you can tune)
DE_POP = 50

F_MUT = 0.6  # mutation factor F
CR = 0.9  # crossover rate
INIT_SIGMA = 0.75  # init spread for Normal(mu, sigma)
...
PRINT_CHANGE_RATIO = True  # kept for compatibility, no longer used for stopping
...
def read_nodes(node_path: str) -> pd.DataFrame:
    ...
    def _pick_col(
        cols: List[str], candidates: List[str], field_name: str, required: bool = True
    ) -> Optional[str]:
        ...
        for cand in candidates:
            key = _norm(cand)
            for ncol, ocol in norm2orig.items():
                if key in ncol or ncol in key:
                    fuzzy_matches.append(ocol)
        ...
        if required:
            raise ValueError(
                f"{node_path} missing required field '{field_name}'. "
                f"columns={list(cols)} candidates={candidates}"
            )
        return None

    cols = list(df.columns)
    id_col = _pick_col(cols, ["id", "node_id", "nodeid", "station_id", "stationid"], "id", required=True)
    lon_col = _pick_col(cols, ["longitude", "lon", "lng", "x", "xcoord", "long"], "longitude", required=True)
    lat_col = _pick_col(cols, ["latitude", "lat", "y", "ycoord"], "latitude", required=True)

    mapped = df.rename(columns={id_col: "id", lon_col: "longitude", lat_col: "latitude"}).copy()
    mapped = mapped[["id", "longitude", "latitude"]].copy()

    mapped["id"] = mapped["id"].astype(int)
    mapped["longitude"] = mapped["longitude"].astype(float)
    mapped["latitude"] = mapped["latitude"].astype(float)

    return mapped.sort_values("id").reset_index(drop=True)
...
def read_edges(edge_path: str) -> pd.DataFrame:
    ...
    def _pick_col(
        ...
        if required:
            raise ValueError(
                f"{edge_path} missing required field '{field_name}'. "
                f"columns={list(cols)} candidates={candidates}"
            )
        return None
    ...
    u_col = _pick_col(
        cols,
        ["u", "from", "source", "src", "start", "origin", "from_node", "tail"],
        "u",
        required=True,
    )
    v_col = _pick_col(
        cols,
        ["v", "to", "target", "dst", "end", "dest", "to_node", "head"],
        "v",
        required=True,
    )
    w_col = _pick_col(
        cols,
        ["weight", "w", "flow", "traffic_flow", "demand", "volume"],
        "weight",
        required=True,
    )
    d_col = _pick_col(
        cols,
        ["distance_km", "distance", "dist_km", "length_km", "distancekm", "km", "length", "dist"],
        "distance_km",
        required=False,
    )

    rename_map = {u_col: "u", v_col: "v", w_col: "weight"}
    if d_col is not None:
        rename_map[d_col] = "distance_km"

    mapped = df.rename(columns=rename_map).copy()
    keep_cols = ["u", "v", "weight"] + (["distance_km"] if "distance_km" in mapped.columns else [])
    mapped = mapped[keep_cols].copy()

    if "distance_km" not in mapped.columns:
        mapped["distance_km"] = 0.0

    mapped["u"] = mapped["u"].astype(int)
    mapped["v"] = mapped["v"].astype(int)
    mapped["weight"] = mapped["weight"].astype(float)
    mapped["distance_km"] = mapped["distance_km"].astype(float)

    uv_min = np.minimum(mapped["u"].values, mapped["v"].values)
    uv_max = np.maximum(mapped["u"].values, mapped["v"].values)
    mapped["_a"], mapped["_b"] = uv_min, uv_max
    mapped = mapped.drop_duplicates(subset=["_a", "_b"]).drop(columns=["_a", "_b"]).reset_index(drop=True)
    return mapped


def filter_edges_and_get_active_nodes(
    all_nodes: List[int], edges_by_scenario: Dict[str, pd.DataFrame], w_min: float = 1.0
    ...
    for sname, edf in edges_by_scenario.items():
        keep = edf[
            (edf["weight"] >= float(w_min)) & (edf["u"].isin(all_set)) & (edf["v"].isin(all_set))
        ].copy()
...
def decode_x_to_plan(x: np.ndarray, idx2nid: List[int]) -> Dict[int, int]:
    plan = {}
...
def evaluate_global_plan(
    ...
    for tname, edf in edges_by_scenario.items():
        ...
        o_vars = {
            idx: pulp.LpVariable(f"o_{idx}", lowBound=0.0, upBound=1.0, cat="Continuous") for idx in sub_edges.index
        }
        s_vars = {nid: pulp.LpVariable(f"s_{nid}", lowBound=0.0, cat="Continuous") for nid in all_nodes_solve}
...
# 7) DE core
# =========================
def init_population(mu: np.ndarray, pop: int, sigma: float) -> np.ndarray:
    X = np.random.normal(loc=mu[None, :], scale=sigma, size=(pop, len(mu))).astype(np.float64)
...
def run_de_global(logger: Optional[RunLogger], seed: int, run_id: int = 1):
    if logger is None:
    ...
    edges_by_scenario, active_nodes, inactive_nodes = filter_edges_and_get_active_nodes(
        all_nodes=all_nodes, edges_by_scenario=edges_by_scenario_raw, w_min=EDGE_WEIGHT_MIN
    )
    ...
    # init
    X = init_population(mu, DE_POP, INIT_SIGMA)
    fit = np.zeros((DE_POP,), dtype=np.float64)
    cost_arr = np.zeros((DE_POP,), dtype=np.float64)
    viol_arr = np.zeros((DE_POP,), dtype=np.float64)

    print("[DE] Evaluate initial population ...", flush=True)
    best_fit = float("inf")
    best_cost = float("inf")
    best_viol = float("inf")
    best_x = None
    ...
    for i in range(DE_POP):
        if (logger is not None) and (logger.fe >= MAX_TOTAL_FE):
            break
        plan = decode_x_to_plan(X[i], idx2nid)
        f, c, v = evaluate_global_plan(all_nodes_solve, edges_by_scenario, plan, BETA_PENALTY, logger=logger)
        fit[i], cost_arr[i], viol_arr[i] = f, c, v
        if f < best_fit - 1e-9:
            best_fit, best_cost, best_viol = f, c, v
            best_x = X[i].copy()

    print(f"[DE] Init snapshot: best_cost={best_cost:.2f}  best_viol={best_viol:.4f}  best_obj={best_fit:.2f}", flush=True)

    # evolve
    print("[DE] Evolve under FE budget ...", flush=True)
    pbar = tqdm(desc=f"DE Generations (run {run_id:02d})", dynamic_ncols=True)
    gen = 0
    while True:
        if (logger is not None) and (logger.fe >= MAX_TOTAL_FE):
            logger.set_stop(
        ...
        for i in range(DE_POP):
            ...
            u = X[i].copy()
            j_rand = random.randrange(n_dim)
            for j in range(n_dim):
                if random.random() < CR or j == j_rand:
                    u[j] = v[j]
            u = np.clip(u, X_MIN, X_MAX)

            plan_u = decode_x_to_plan(u, idx2nid)
            f_u, c_u, v_u = evaluate_global_plan(all_nodes_solve, edges_by_scenario, plan_u, BETA_PENALTY, logger=logger)
            ...
            if f_u <= fit[i] + 1e-12:
                X[i] = u
                fit[i], cost_arr[i], viol_arr[i] = f_u, c_u, v_u

                if f_u < best_fit - 1e-9:
                    best_fit, best_cost, best_viol = f_u, c_u, v_u
                    best_x = u.copy()
                    gen_improved = True

        gen_best_idx = int(np.argmin(fit))
        gen_best_fit = float(fit[gen_best_idx])
        gen_best_cost = float(cost_arr[gen_best_idx])
        gen_best_viol = float(viol_arr[gen_best_idx])
        gen_avg_fit = float(np.mean(fit))

        if logger is not None:
            logger.record_global_gen_point(gen_best_fit, gen_best_cost, gen_best_viol)

        pbar.set_postfix(
            {
                "gen_best": f"{gen_best_fit:.2e}",
                "gen_cost": f"{gen_best_cost:.2e}",
                "gen_viol": f"{gen_best_viol:.2e}",
                "best": f"{best_fit:.2e}",
                "avg": f"{gen_avg_fit:.2e}",
        ...
        if (PRINT_EVERY_GEN is not None) and (PRINT_EVERY_GEN > 0) and (gen % PRINT_EVERY_GEN == 0):
            print(
                ...
                f"gen_best: cost={gen_best_cost:.2f}  viol={gen_best_viol:.4f}  obj={gen_best_fit:.2f} | "
                f"best_so_far: cost={best_cost:.2f}  viol={best_viol:.4f}  obj={best_fit:.2f} "
                f"{'(improved)' if gen_improved else ''}",
                flush=True,
            )
    ...
    best_plan = decode_x_to_plan(best_x, idx2nid)
    best_obj = best_fit

    print(
        f"\n[Run {run_id:02d}] best_global_cost={best_cost:.4f}, best_global_viol={best_viol:.6f}, best_global_obj={best_obj:.4f}",
        flush=True,
    )
    ...
    out["cap_2020"] = out["scale_id"].map(lambda sid: C_sess_day[int(sid)] * SCENARIO_DAYS["2020"])
    out["cap_2025"] = out["scale_id"].map(lambda sid: C_sess_day[int(sid)] * SCENARIO_DAYS["2025"])
    out["cap_2030"] = out["scale_id"].map(lambda sid: C_sess_day[int(sid)] * SCENARIO_DAYS["2030"])
    out["cap_2035"] = out["scale_id"].map(lambda sid: C_sess_day[int(sid)] * SCENARIO_DAYS["2035"])
    out["cap_2040"] = out["scale_id"].map(lambda sid: C_sess_day[int(sid)] * SCENARIO_DAYS["2040"])

    out["build_cost_usd"] = out["scale_id"].map(lambda sid: build_cost[int(sid)])
    total_days = sum(SCENARIO_DAYS.values())
    out["op_cost_usd_for_total_duration"] = out["scale_id"].map(lambda sid: op_cost_yr[int(sid)] * (total_days / 365.0))
...
def run_one(
    ...
    try:
        ...
        best_plan, best_obj, best_cost, best_viol, plan_path = run_de_global(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run DE global baseline.")
