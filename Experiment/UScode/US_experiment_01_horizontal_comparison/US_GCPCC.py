...
LOG_DIR = os.path.join(SCRIPT_DIR, "gcpcc_results")
...
def run_gcpcc(
    ...
    # Decomposition pipeline (aligned with US Spectralcut-Visual-US.py):
    # edge filtering -> spatial cut -> fragment repair -> consensus cut -> small component merge.
    ...
    while True:
        ...
        if PARALLEL_WORKERS > 1 and len(comps_to_opt) > 1:
            ...
            for ci in tqdm(comps_to_opt, desc=f"Run {run_id:02d} Gen {outer_gen} components", leave=False, dynamic_ncols=True):
                comp_nodes = components[ci]
                comp_seed = (seed * 1000003 + outer_gen * 1009 + ci * 9176) % (2**31 - 1)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run original GCPCC with generation hard cap.")
