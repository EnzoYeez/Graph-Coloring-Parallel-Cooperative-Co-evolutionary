...
_DATA_DIR = os.path.join(_PROJECT_DIR, "US_拓扑图数据")
...
def make_random_groups_equal_size(
    ...
    """
    Randomly permute all variables, then split into groups.
    If n is not divisible, remainder nodes are distributed to some groups.
    """
    ...
    if s is not None and m is not None:
        ...
        s = max(1, min(int(s), n))
        m = int(math.ceil(n / float(s)))
    elif m is not None:
        m = max(1, min(int(m), n))
        s = n // m
    else:
        # Default: choose m near sqrt(n).
        m0 = max(2, int(round(math.sqrt(n))))
        m = max(1, min(int(m0), n))
        s = n // m

    # Nearly-even size allocation (difference at most 1).
    base = n // m
    rem = n % m
    sizes = [base + 1] * rem + [base] * (m - rem)
    if not sizes or sum(sizes) != n:
        raise ValueError(f"Internal error: invalid group sizes for n={n}, m={m}.")
    ...
    for gsz in sizes:
        ed = st + int(gsz)
        groups.append(sorted(perm[st:ed]))
        st = ed
    if st != n:
        raise ValueError(f"Internal error: grouping coverage mismatch st={st}, n={n}.")

    s_nominal = int(math.floor(n / m))
    return groups, m, s_nominal
...
def build_components_for_mode(
    ...
    if mode == DECOMP_MODE_RANDOM_GROUPING_RR:
        ...
        # Real random grouping is handled in run_gcpcc for consistent initialization.
        return [_sort_components([all_nodes_solve])[0]]
    ...
    raise ValueError(f"Unsupported decomp_mode in US_RGCC: {mode}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="US_RG ablation: random grouping decomposition + GCPCC-aligned downstream optimization."
    )
    parser.add_argument("--run_id", type=int, default=1)
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--output_root", type=str, default=os.path.join(SCRIPT_DIR, "gcpcc_logs_US_RG"))
