...
def run_gcpcc(
    ...
    print("[1/5] Load scenarios", flush=True)
    ...
    print("[2/5] Direct sub-optimizer on full active graph", flush=True)
    comp_nodes = sorted(int(nid) for nid in all_nodes_solve)
    print(f"  component_size={len(comp_nodes)} (no decomposition)", flush=True)

    # SA-like roulette initialization.
    print("[3/5] Initialize plan", flush=True)
    rng = random.Random(seed)
    ...
    print("[4/5] Outer iterations", flush=True)
    outer_gen = 0
    ...
    while True:
        ...
        print(f"\n[Run {run_id:02d} Gen {outer_gen}] direct_component=1", flush=True)
        shared_plan = dict(X)
        context_cache = build_context_allocation_cache(all_nodes_solve, edges_by_scenario, shared_plan)

        comp_seed = (seed * 1000003 + outer_gen * 1009 + 9176) % (2**31 - 1)
        updates, fe_this_gen_single_comp = ga_optimize_component(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="US_wo direct mode: optimize all active nodes with the sub-optimizer (no decomposition)."
    )
    parser.add_argument("--run_id", type=int, default=1)
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--output_root", type=str, default=os.path.join(SCRIPT_DIR, "gcpcc_logs_US_wo"))
