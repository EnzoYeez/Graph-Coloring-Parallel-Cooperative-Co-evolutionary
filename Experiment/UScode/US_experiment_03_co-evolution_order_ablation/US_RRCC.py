...
LOG_DIR = os.path.join(SCRIPT_DIR, "gcpcc_logs_US_RR")
...
def ga_optimize_component(
    ...
    generations: int,
    beta_penalty: float,
 ) -> Tuple[Dict[int, Tuple[int, int, int]], int, float, float, float]:
    ...
    base_fit, base_cost, base_viol = evaluate_global_plan(
        all_nodes, edges_by_scenario, base_plan, beta_penalty, logger=logger
    )
    ...
    for _gen in range(generations):
        scored: List[Tuple[float, float, float, Dict[int, Tuple[int, int, int]]]] = []
        for ind in population:
            fit, cost, viol = evaluate_global_plan(
                all_nodes, edges_by_scenario, ind, beta_penalty, logger=logger
            )
            scored.append((fit, cost, viol, ind))
            local_fe_count += 1
        ...
        if scored[0][0] < best_fit:
            ...
            best_viol = scored[0][2]
            best_ind = dict(scored[0][3])

        selection_pool = [(fit, ind) for fit, _cost, _viol, ind in scored]
        new_pop = [dict(best_ind)]
        while len(new_pop) < pop_size:
            p1 = tournament_select(selection_pool, rng, k=min(TOURNAMENT_K, len(selection_pool)))
            p2 = tournament_select(selection_pool, rng, k=min(TOURNAMENT_K, len(selection_pool)))
    ...
    if best_fit > base_fit + 1e-12:
        return (
            {nid: shared_plan[nid] for nid in comp_nodes},
            int(local_fe_count),
            base_fit,
            base_cost,
            base_viol,
...
def run_gcpcc(
    ...
    print("[4/7] Build serial component order", flush=True)
    ...
    while True:
        ...
        ci = component_order[(outer_gen - 1) % len(component_order)]
        comp_nodes = components[ci]
        print(f"\n[Run {run_id:02d} Gen {outer_gen}] comp={ci + 1}/{len(component_order)} size={len(comp_nodes)}", flush=True)

        shared_plan = dict(X)

        comp_seed = (seed * 1000003 + outer_gen * 1009 + ci * 9176) % (2**31 - 1)
        updates, comp_fe, global_obj, global_cost, global_viol = ga_optimize_component(
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run RRCC with the same GCPCC core and a serial round-robin component update order.")
