...
# Keep an independent output folder per serial-schedule variant under this script directory.
SCRIPT_DIR = os.path.dirname(os.path.abspath(__file__))
LOG_DIR = os.path.join(SCRIPT_DIR, "gcpcc_logs_US_FR")
...
# ---- Consensus Cut (Version A, aligned with Spectralcut-Visual-US.py) ----
...
# ---- FRCC scheduling knobs (only evolution order differs from GCPCC) ----
FRCC_CONTRIB_OLD_WEIGHT = 0.50
FRCC_CONTRIB_EQUAL_EPS = 1e-12
...
def tournament_select(
    scored: List[Tuple[float, Dict[int, Tuple[int, int, int]], float, float]],
    rng: random.Random,
    k: int = 2
):
...
def ga_optimize_component(
    ...
    generations: int,
    beta_penalty: float,
    current_global_metrics: Tuple[float, float, float],
    context_cache: dict,
) -> Tuple[Dict[int, Tuple[int, int, int]], float, float, float, int]:
    ...
    best_fit, best_cost, best_viol = current_global_metrics
    local_fe_count = 0

    for _gen in range(generations):
        scored: List[Tuple[float, Dict[int, Tuple[int, int, int]], float, float]] = []
        for ind in population:
            fit, global_cost, global_viol = evaluate_global_plan(
                all_nodes, edges_by_scenario, ind, beta_penalty, logger=logger
            )
            scored.append((fit, ind, global_cost, global_viol))
            local_fe_count += 1

        scored.sort(key=lambda x: x[0])
        if scored[0][0] < best_fit - 1e-12:
            best_fit = scored[0][0]
            best_ind = dict(scored[0][1])
            best_cost = scored[0][2]
            best_viol = scored[0][3]
    ...
    return (
        {nid: best_ind[nid] for nid in comp_nodes},
        float(best_fit),
        float(best_cost),
        float(best_viol),
        int(local_fe_count),
...
def run_gcpcc(
    ...
    for edf in edges_by_scenario.values():
        for _, r in edf.iterrows():
            ...
            if w <= 0:
                continue
            if u in d_raw: d_raw[u] += w
            if v in d_raw: d_raw[v] += w

    # Decomposition pipeline.
    print("[2/7] Spatial cut + fragment repair", flush=True)
    scenario_clusters: Dict[str, List[List[int]]] = {}
    scenario_node_labels: Dict[str, Dict[int, int]] = {}
    ...
    for sname, sg in scenarios.items():
        ...
        scenario_clusters[sname] = clusters_repaired
    ...
    min_agree_eff = resolve_consensus_min_agree(
        n_scenarios=len(scenario_names),
        min_agree=CONSENSUS_MIN_AGREE,
        min_agree_ratio=CONSENSUS_MIN_AGREE_RATIO
    )
    print(
        f"[3/7] Consensus cut (min_agree={min_agree_eff}, "
        f"cfg_min={CONSENSUS_MIN_AGREE}, cfg_ratio={CONSENSUS_MIN_AGREE_RATIO})",
        flush=True
    )
    ...
    if sizes:
        ...
        raise RuntimeError("Consensus cut returned empty components. Check labels/edges.")
    ...
    print(f"  serial_components={len(component_order)}", flush=True)
    ...
    pbar = tqdm(desc=f"Outer Generations (run {run_id:02d})", dynamic_ncols=True)
    outer_gen = 0
    current_obj, current_cost, current_viol = init_obj, init_cost, init_viol

    component_contrib = {ci: 0.0 for ci in component_order}
    force_exploration = True
    explore_queue: List[int] = list(component_order)
    ...
    while True:
        ...
        while True:
            if force_exploration:
                if len(explore_queue) == 0:
                    ...
                    continue
                explore_pos = len(component_order) - len(explore_queue) + 1
                ci = int(explore_queue.pop(0))
                phase_tag = f"explore[{explore_pos}/{len(component_order)}]"
                break

            contrib_vals = [float(component_contrib[c]) for c in component_order]
            contrib_equal = (max(contrib_vals) - min(contrib_vals)) <= FRCC_CONTRIB_EQUAL_EPS
            if contrib_equal:
                force_exploration = True
                explore_queue = list(component_order)
                print(
                    f"[Run {run_id:02d} Gen {outer_gen}] explore_restart=1 (contrib nearly equal)",
                    flush=True,
                )
                continue

            ci = component_order[0]
            best_contrib = float(component_contrib[ci])
            for cand in component_order[1:]:
                val = float(component_contrib[cand])
                if val > best_contrib:
                    best_contrib = val
                    ci = cand
            phase_tag = "focus[1/1]"
        ...
        print(f"\n[Run {run_id:02d} Gen {outer_gen}] {phase_tag} comp={ci + 1}/{len(component_order)} size={len(comp_nodes)}", flush=True)

        shared_plan = dict(X)
        context_cache = build_context_allocation_cache(all_nodes_solve, edges_by_scenario, shared_plan)
        before_global_obj = float(current_obj)

        comp_seed = (seed * 1000003 + outer_gen * 1009 + ci * 9176) % (2**31 - 1)
        updates, candidate_global_obj, candidate_global_cost, candidate_global_viol, comp_fe = ga_optimize_component(
            comp_nodes=comp_nodes,
            all_nodes=all_nodes_solve,
            ...
            current_global_metrics=(current_obj, current_cost, current_viol),
            context_cache=context_cache,
        )

        X_candidate = dict(X)
        for nid, seg in updates.items():
            X_candidate[int(nid)] = tuple(seg)

        if logger is not None:
            logger.add_fe(comp_fe)

        improve = max(0.0, float(before_global_obj - candidate_global_obj))
        if candidate_global_obj < current_obj - 1e-12:
            X = X_candidate
            current_obj = float(candidate_global_obj)
            current_cost = float(candidate_global_cost)
            current_viol = float(candidate_global_viol)

        old_contrib = float(component_contrib[int(ci)])
        component_contrib[int(ci)] = (
            FRCC_CONTRIB_OLD_WEIGHT * old_contrib
            + (1.0 - FRCC_CONTRIB_OLD_WEIGHT) * improve
        )

        fe_now = (logger.fe if logger is not None else 0)
        print(
            f"[Run {run_id:02d} Gen {outer_gen}] "
            f"fe+={int(comp_fe)} fe={int(fe_now)} "
            f"cost={current_cost:.2f} viol={current_viol:.4f} obj={current_obj:.2f} "
            f"delta_obj={improve:.6f} contrib={component_contrib[int(ci)]:.6f}",
            flush=True
        )

        if logger is not None:
            logger.record_global_gen_point(current_obj, current_cost, current_viol)

        if current_obj < best_obj - 1e-9:
            best_obj, best_cost, best_viol = current_obj, current_cost, current_viol
            best_X = dict(X)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run FRCC with the same GCPCC core and a serial feedback-priority update order.")
