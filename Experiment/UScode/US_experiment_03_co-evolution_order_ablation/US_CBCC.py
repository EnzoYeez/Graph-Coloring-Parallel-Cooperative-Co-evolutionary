...
LOG_DIR = os.path.join(SCRIPT_DIR, "gcpcc_logs_US_CB")
...
# ---- CBCC scheduling knobs (only evolution order differs from GCPCC) ----
CBCC_ETA_OLD_WEIGHT = 0.35
CBCC_REWARD_RATIO = 0.5
CBCC_ETA_EPS = 1e-12
...
def ga_optimize_component(
    ...
    for _gen in range(generations):
        ...
        for ind in population:
            global_obj, _global_cost, _global_viol = evaluate_global_plan(
                all_nodes_solve, edges_by_scenario, ind, beta_penalty, logger=logger
            )
            scored.append((global_obj, ind))
            global_fe_count += 1
    ...
    return {nid: best_ind[nid] for nid in comp_nodes}, int(global_fe_count)
...
def run_gcpcc(
    ...
    component_eta = {ci: 0.0 for ci in component_order}
    phase = "rr"
    rr_cursor = 0
    reward_queue: List[int] = []
    reward_pos = 0
    ...
    while True:
        ...
        while True:
            if phase == "rr":
                ci = component_order[rr_cursor]
                rr_cursor += 1
                phase_tag = f"rr[{rr_cursor}/{len(component_order)}]"
                if rr_cursor >= len(component_order):
                    phase = "reward_ready"
                break

            if phase == "reward_ready":
                eta_max = max(float(component_eta[c]) for c in component_order)
                reward_threshold = float(CBCC_REWARD_RATIO * eta_max)
                first_echelon = [
                    int(c) for c in component_order
                    if float(component_eta[int(c)]) + CBCC_ETA_EPS >= reward_threshold
                ]
                if len(first_echelon) == 0 or len(first_echelon) == len(component_order):
                    print(
                        f"[Run {run_id:02d} Gen {outer_gen}] reward_skipped=1 "
                        f"(first_echelon={len(first_echelon)}/{len(component_order)}, eta_max={eta_max:.6f})",
                        flush=True,
                    )
                    phase = "rr"
                    rr_cursor = 0
                ...
                reward_queue = first_echelon
                reward_pos = 0
                phase = "reward"
                print(
                    f"[Run {run_id:02d} Gen {outer_gen}] reward_pool={len(reward_queue)}/{len(component_order)} "
                    f"threshold={reward_threshold:.6f} eta_max={eta_max:.6f}",
                    flush=True,
                )
                continue

            ci = reward_queue[reward_pos]
            reward_pos += 1
            phase_tag = f"reward[{reward_pos}/{len(reward_queue)}]"
            if reward_pos >= len(reward_queue):
                phase = "rr"
                rr_cursor = 0
            break
        ...
        updates, comp_fe = ga_optimize_component(
        ...
        for nid, seg in updates.items():
            candidate_X[int(nid)] = tuple(seg)

        if logger is not None:
            logger.add_fe(comp_fe)

        cand_obj, cand_cost, cand_viol = evaluate_global_plan(
            all_nodes_solve, edges_by_scenario, candidate_X, BETA_PENALTY, logger=logger
        )
        raw_improve = float(before_global_obj - cand_obj)
        accepted = bool(cand_obj < before_global_obj)
        if accepted:
            X = candidate_X
            current_obj, current_cost, current_viol = cand_obj, cand_cost, cand_viol

        improve_now = max(0.0, raw_improve)
        old_eta = float(component_eta[int(ci)])
        component_eta[int(ci)] = max(0.0, (
            CBCC_ETA_OLD_WEIGHT * old_eta
            + (1.0 - CBCC_ETA_OLD_WEIGHT) * improve_now
        ))
        ...
        print(
            ...
            f"cand_obj={cand_obj:.2f} raw_improve={raw_improve:.6f} "
            f"accepted={int(accepted)} eta={component_eta[int(ci)]:.6f}",
            flush=True
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run CBCC with the same GCPCC core and a serial RR + first-echelon reward update order.")
