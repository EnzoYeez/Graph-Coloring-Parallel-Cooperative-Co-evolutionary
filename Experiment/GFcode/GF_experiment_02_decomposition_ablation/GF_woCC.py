...
DEFAULT_DECOMP_MODE = DECOMP_MODE_SINGLE_COMPONENT
...
def run_one(
    ...
    output_root: str = os.path.join(SCRIPT_DIR, "gcpcc_logs_GF_wo"),
    max_outer_gen: Optional[int] = MAX_OUTER_GEN,
):
    run_id = int(run_id)
    seed = _resolve_seed(run_id, seed)
    output_root = output_root or os.path.join(SCRIPT_DIR, "gcpcc_logs_GF_wo")
    ...
    best_X, best_obj, best_cost, best_viol, plan_path = run_gcpcc(
        ...
        seed=seed,
        run_id=run_id,
        decomp_mode=DECOMP_MODE_SINGLE_COMPONENT,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GF_wo ablation: single-component decomposition + GCPCC-aligned downstream optimization."
    )
    parser.add_argument("--run_id", type=int, default=1)
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--output_root", type=str, default=os.path.join(SCRIPT_DIR, "gcpcc_logs_GF_wo"))
