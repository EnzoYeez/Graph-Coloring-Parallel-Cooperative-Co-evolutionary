...
# RG default: split into 5 groups in preprocessing.
RG_NUM_GROUPS_DEFAULT = 5
...
DEFAULT_DECOMP_MODE = DECOMP_MODE_RANDOM_GROUPING_RR
...
def run_gcpcc(
    ...
    rg_num_groups: Optional[int] = RG_NUM_GROUPS_DEFAULT,
    ...
    components: List[List[int]] = []
    rg_rng: Optional[random.Random] = None
    if decomp_mode == DECOMP_MODE_DG_STATIC_RR:
        ...
        print("[2/7] Decomposition: random grouping (cycle-wise regrouping)", flush=True)
        _dummy_groups, rg_m, rg_s = make_random_groups_equal_size(
            all_nodes_solve=all_nodes_solve,
            rng=random.Random(seed + 7919),
            group_size=rg_group_size,
            num_groups=rg_num_groups,
        )
        print(f"  n={len(all_nodes_solve)} m={rg_m} s={rg_s} (n=m*s)", flush=True)
        components = [_sort_components([all_nodes_solve])[0]]
        rg_rng = random.Random(seed * 104729 + 17)
    else:
    ...
    if decomp_mode == DECOMP_MODE_RANDOM_GROUPING_RR:
        color_groups = {}
        group_ids = []
        rg_cycle_components: List[List[int]] = []
        rg_color_groups_cycle: Dict[int, List[int]] = {}
        rg_group_ids_cycle: List[int] = []
        rg_group_cursor = 0
        rg_cycle_idx = 0
        print("  random regroup + recolor after each full color-cycle", flush=True)
    else:
    ...
    while True:
        ...
        if decomp_mode == DECOMP_MODE_RANDOM_GROUPING_RR:
            if rg_group_cursor >= len(rg_group_ids_cycle):
                cycle_groups, m_now, s_now = make_random_groups_equal_size(
                    all_nodes_solve=all_nodes_solve,
                    rng=rg_rng,
                    group_size=rg_group_size,
                    num_groups=rg_num_groups,
                )
                rg_cycle_components = _sort_components(cycle_groups)
                H_cycle = build_dependency_graph(rg_cycle_components, all_edges_union)
                rg_color_groups_cycle = greedy_coloring_groups(H_cycle)
                rg_group_ids_cycle = sorted(rg_color_groups_cycle.keys())
                if not rg_group_ids_cycle:
                    raise RuntimeError("Random regrouping produced empty color groups.")
                rg_group_cursor = 0
                rg_cycle_idx += 1
                print(
                    f"[Run {run_id:02d} Gen {outer_gen}] regrouped=1 cycle={rg_cycle_idx} groups={m_now} group_size={s_now} color_groups={len(rg_group_ids_cycle)}",
                    flush=True,
                )

            group = rg_group_ids_cycle[rg_group_cursor]
            comps_to_opt = rg_color_groups_cycle[group]
            components_use = rg_cycle_components
            rg_group_cursor += 1
        else:
            group = group_ids[(outer_gen - 1) % len(group_ids)]
            comps_to_opt = color_groups[group]
            components_use = components
...
def run_one(
    ...
    output_root: str = os.path.join(SCRIPT_DIR, "gcpcc_logs_GF_RG"),
    rg_group_size: Optional[int] = None,
    rg_num_groups: Optional[int] = RG_NUM_GROUPS_DEFAULT,
    max_outer_gen: Optional[int] = MAX_OUTER_GEN,
):
    ...
    output_root = output_root or os.path.join(SCRIPT_DIR, "gcpcc_logs_GF_RG")
    ...
    best_X, best_obj, best_cost, best_viol, plan_path = run_gcpcc(
        ...
        seed=seed,
        run_id=run_id,
        decomp_mode=DECOMP_MODE_RANDOM_GROUPING_RR,
        log_dir=run_output_dir,
        rg_group_size=rg_group_size,
        rg_num_groups=rg_num_groups,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="GF_RG ablation: random grouping decomposition + GCPCC-aligned downstream optimization."
    )
    parser.add_argument("--run_id", type=int, default=1)
    parser.add_argument("--seed", type=int, default=None)
    parser.add_argument("--output_root", type=str, default=os.path.join(SCRIPT_DIR, "gcpcc_logs_GF_RG"))
    parser.add_argument("--n_runs", type=int, default=1)
    parser.add_argument("--base_seed", type=int, default=2026)
    parser.add_argument("--rg_group_size", type=int, default=None)
    parser.add_argument("--rg_num_groups", type=int, default=RG_NUM_GROUPS_DEFAULT)
    ...
    if int(args.n_runs) > 1:
        run_many(
            ...
            rg_group_size=args.rg_group_size,
            rg_num_groups=args.rg_num_groups,
